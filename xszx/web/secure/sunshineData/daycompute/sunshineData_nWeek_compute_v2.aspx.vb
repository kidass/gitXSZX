...
Namespace Xydc.Platform.web
    ...
    Partial Public Class sunshineData_nWeek_compute_v2
        ...
        '定义日楼盘数据时间
        Private dayHouseDate As DateTime
...
errProc:
        ...
        Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
            ...
            Try
                '查证是否选择时间
                If Me.txtStartDate.Text.Trim = "" Or Me.txtEndStartDate.Text.Trim = "" Then
                    strErrMsg = "提示：时间段选择错误,请查证！"
                    GoTo errProc
                End If
...
errProc:
        ...
        Private Function getModuleData_Compute( _
            ...
            Dim strTable As String = Xydc.Platform.Common.Data.SunshineData.TABLE_Sunshine_V_NWeek_Statistics
            ...
            Try
                ...
                Dim strEndStartDate As String

                datStartDate = CDate(Me.txtStartDate.Text)
                'datStartDate = datStartDate.AddDays(-1)
                strStartDate = CStr(datStartDate)
                If Me.txtEndStartDate.Text.Trim = "" Then
                    strEndStartDate = strStartDate
                Else
                    'strEndStartDate = CStr(CDate(Me.txtEndStartDate.Text).AddDays(-1))
                    strEndStartDate = Me.txtEndStartDate.Text
                End If


                objParameter.Clear()
                objParameter.Add("StartDate", strStartDate)
                objParameter.Add("EndDate", strEndStartDate)
                ...
                If objsystemSunshine.getDataSet_NWeek_Compute_v2(strErrMsg, MyBase.UserId, MyBase.UserPassword, strWhere, objParameter, Me.m_objDataSet_Compute) = False Then
                    GoTo errProc
                End If
        ...
        Private Function showDataGridInfo_Compute( _
            ...
            Try
                ...
                If rblTop.SelectedValue = "0" Then
                    Me.grdCompute.Columns(2).Visible = True
                    Me.grdCompute.Columns(4).Visible = True
                    Me.grdCompute.Columns(6).Visible = True
                    Me.grdCompute.Columns(1).Visible = False
                    Me.grdCompute.Columns(3).Visible = False
                    Me.grdCompute.Columns(5).Visible = False
                Else
                    Me.grdCompute.Columns(2).Visible = False
                    Me.grdCompute.Columns(4).Visible = False
                    Me.grdCompute.Columns(6).Visible = False
                    Me.grdCompute.Columns(1).Visible = True
                    Me.grdCompute.Columns(3).Visible = True
                    Me.grdCompute.Columns(5).Visible = True
                End If
            ...
            If Me.IsPostBack = False Then
                Try
                    ...
                    With New Xydc.Platform.BusinessFacade.systemSunshine_x2
                        If .getDayHouseDate(strErrMsg, MyBase.UserId, MyBase.UserPassword, dayHouseDate) = False Then
                            GoTo errProc
                        End If
                    End With
                    Me.txtEndDate.Text = dayHouseDate.ToShortDateString
...
errProc:
        ...
        Private Sub doPrint(ByVal strControlId As String)
            ...
            Try
                ...
                'datStartDate = datStartDate.AddDays(-1)
                strStartDate = CStr(datStartDate)
                'strEndStartDate = CStr(CDate(Me.htxtEndDate.Value).AddDays(-1))
                strEndStartDate = CStr(CDate(Me.htxtEndDate.Value))
                ...
                If objsystemSunshine.getDataSet_NWeek_Compute_v2(strErrMsg, MyBase.UserId, MyBase.UserPassword, Me.m_strQuery_Compute, objParameter, Me.m_objDataSet_Compute) = False Then
                    GoTo errProc
                End If
                ...
                Dim strSrcExcelSpec As String
                If rblTop.SelectedValue = "0" Then
                    strSrcExcelSpec = Request.ApplicationPath + Me.m_cstrExcelMBRelativePathToAppRoot + "N周区域价格表.xls"
                Else
                    strSrcExcelSpec = Request.ApplicationPath + Me.m_cstrExcelMBRelativePathToAppRoot + "N周区域套数表.xls"
                End If
...
errProc:
        ...
        Private Sub lnkRq_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkRq.Click
            ...
            '结束周日期
            If Me.txtEndRQ.Text.Trim <> "" Then
                dateNow = CType(Me.txtEndRQ.Text, System.DateTime)
                strTemp = ""
                If objPulicParameters.getWeekStartAndEndDay(strErrMsg, dateNow, date1, date2, "") = True Then
                    Me.txtEndStartDate.Text = date1.ToString("yyyy-MM-dd")
                    Me.txtEndEndDate.Text = date2.ToString("yyyy-MM-dd")
                End If
            End If
            ...
            If Me.txtStartDate.Text.Trim <> "" And Me.txtEndStartDate.Text.Trim <> "" Then
                intInterval = DateDiff(DateInterval.Weekday, CType(Me.txtStartDate.Text, System.DateTime), CType(Me.txtEndStartDate.Text, System.DateTime))
                If intInterval < 0 Then
                    Me.txtEndStartDate.Text = ""
                    Me.txtEndEndDate.Text = ""
                    txtWeeks.Text = ""
                    strErrMsg = "提示：开始周日期小于结束周日期，请查证！"
                    GoTo errProc
                Else
                    txtWeeks.Text = CStr(intInterval + 1)
                End If
            End If
            Exit Sub
errProc:
        ...
        '输入周数，并输入一个开始周，然后计算N周后的日期
        Private Sub btnSetWeekStart_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSetWeekStart.Click
            Dim objMessageProcess As New Xydc.Platform.web.MessageProcess
            Dim strErrMsg As String
            ...
            '验证周数
            If Me.txtWeeks.Text.Trim <> "" Then
                Try
                    intWeeks = CInt(Me.txtWeeks.Text.Trim()) - 1
                Catch ex As Exception
                    strErrMsg = "提示：周数输入错误的数字，请查证！"
                    GoTo errProc
                End Try
            Else
                strErrMsg = "提示：周数不能为空，请查证！"
                GoTo errProc
            End If

            '开始周日期
            If Me.txtStartDate.Text.Trim <> "" Then
                dateNow = CType(Me.txtStartDate.Text, System.DateTime)
                dateNow = dateNow.AddDays(7 * intWeeks)
                If objPulicParameters.getWeekStartAndEndDay(strErrMsg, dateNow, date1, date2, "") = True Then
                    Me.txtEndStartDate.Text = date1.ToString("yyyy-MM-dd")
                    Me.txtEndEndDate.Text = date2.ToString("yyyy-MM-dd")
                End If
            Else
                strErrMsg = "提示：开始周日期不能为空，请查证！"
                GoTo errProc
            End If
...
errProc:
        ...
        Private Sub btnSetWeekEnd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSetWeekEnd.Click
            Dim objMessageProcess As New Xydc.Platform.web.MessageProcess
            Dim strErrMsg As String
            Dim intWeeks As Integer
            ...
            '结束周日期
            If Me.txtEndStartDate.Text.Trim <> "" Then
                dateNow = CType(Me.txtEndStartDate.Text, System.DateTime)
                dateNow = dateNow.AddDays(-7 * intWeeks)
                If objPulicParameters.getWeekStartAndEndDay(strErrMsg, dateNow, date1, date2, "") = True Then
                    Me.txtStartDate.Text = date1.ToString("yyyy-MM-dd")
                    Me.txtEndDate.Text = date2.ToString("yyyy-MM-dd")
