...
Namespace Xydc.Platform.web
    ...
    '----------------------------------------------------------------
    ' 命名空间：Xydc.Platform
    ' 类名    ：sunshineData_houseInfo_compute
    ' 
    ' 调用性质：
    '     I/O
    '
    ...
    Partial Public Class sunshineData_houseDetail_compute
        ...
        Private Function getQueryString_Compute( _
            ...
            Try
                ...
                If Me.txtStartDate.Text <> "" And Me.txtEndDate.Text <> "" Then
                    ...
                    '当统计结束日期大于当前数据库最新记录的时间时，取数据库当前最大日期
                    With New Xydc.Platform.BusinessFacade.systemSunshine_x2
                        ...
                        End If
                    End With
                    If dateMax > dayHouseDataTime Then
                ...
                strTxtTemp = ""
                strSearchFied = "a.预售证"
                strTxtTemp = Me.txtPresellId.Text()
                If strTxtTemp <> "" Then
                    strTxtTemp = objPulicParameters.getNewSearchString(strTxtTemp)
                    If strQuery = "" Then
                        strQuery = strSearchFied + " like '" + strTxtTemp + "%'"
                ...
                '楼盘匹配
                If ddlBuildingMatch.SelectedValue = "0" Then
                    '按楼盘名称
                    strSearchFied = ""
                    strTxtTemp = ""
                ...
                Else
                    ...
                    strSearchFied = "a.楼盘名称"
                    If strQuery = "" Then
                        Select Case Me.ddlBuildingMatch.SelectedValue
                            Case "1"
                                If Me.ddlHouseType.SelectedValue = "0" Then
                                    strQuery = strSearchFied + " in (select BuildingName from Sunshine_B_MonthMonitoringHouse) "
                                Else
                                    strTxtTemp = Me.ddlHouseType.SelectedValue
                                    strQuery = strSearchFied + " in (select BuildingName from Sunshine_B_MonthMonitoringHouse where type like '%" + strTxtTemp + "%') "
                                End If
                            Case "2"
                                If Me.ddlHouseType.SelectedValue = "0" Then
                                ...
                                Else
                                    strTxtTemp = Me.ddlHouseType.SelectedValue
                                    strQuery = strSearchFied + " in (select C_NAME from T_HOUSE_MATCH_SORT where I_Type=convert(integer," + strTxtTemp + ")) "
                                End If
                            Case Else
                        End Select
                    Else
                        Select Case Me.ddlBuildingMatch.SelectedValue
                            Case "1"
                                If Me.ddlHouseType.SelectedValue = "0" Then
                                    strQuery = strQuery + " and " + strSearchFied + " in (select BuildingName from Sunshine_B_MonthMonitoringHouse) "
                                Else
                                    strTxtTemp = Me.ddlHouseType.SelectedValue
                                    strQuery = strQuery + " and " + strSearchFied + " in (select BuildingName from Sunshine_B_MonthMonitoringHouse where type like '%" + strTxtTemp + "%') "
                                End If
                            Case "2"
                                If Me.ddlHouseType.SelectedValue = "0" Then
                                ...
                                Else
                                    strTxtTemp = Me.ddlHouseType.SelectedValue
                                    strQuery = strQuery + " and " + strSearchFied + " in (select C_NAME from T_HOUSE_MATCH_SORT where I_Type=convert(integer," + strTxtTemp + ")) "
                                End If
                            Case Else
                        End Select
...
errProc:
        ...
        Private Function getModuleData_Compute( _
            ...
            Dim strTable As String = Xydc.Platform.Common.Data.SunshineData.TABLE_Sunshine_V_Houseinfo_Statistics
            ...
            Try
                ...
                '判断是否已经选择日期，否则提示错误
                If Me.txtStartDate.Text = "" And Me.txtEndDate.Text = "" Then
                    strErrMsg = "错误：请选择查询时间！"
                    GoTo errProc
                ElseIf Me.txtStartDate.Text = "" And Me.txtEndDate.Text <> "" Then
                    Me.txtStartDate.Text = Me.txtEndDate.Text
                ElseIf Me.txtStartDate.Text <> "" And Me.txtEndDate.Text = "" Then
                    Me.txtEndDate.Text = Me.txtStartDate.Text
                End If

                If Me.txtBuildingName.Text = "" And Me.txtPresellId.Text = "" Then
                    strErrMsg = "错误：请输入楼盘名称或者预售证！"
                    GoTo errProc
                End If
                ...
                If objsystemSunshine.getDataSet_BuildingDetailCompute(strErrMsg, MyBase.UserId, MyBase.UserPassword, strWhere, objParameter, Me.m_objDataSet_Compute) = False Then
                    GoTo errProc
                End If
            ...
            Try
                ...
                With New Xydc.Platform.DataAccess.dacSystemOperate
                    If .doSaveOperateLogData(strErrMsg, MyBase.UserId, MyBase.UserPassword, Request.UserHostAddress, Request.UserHostName, _
                        Xydc.Platform.Common.Data.LogData.OperateType_select, Xydc.Platform.Common.Data.SunshineData.TABLE_Sunshine_V_Houseinfo_Statistics, strQuery) = False Then
                        GoTo errProc
                    End If
                End With
            Catch ex As Exception
...
errProc:
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Me.IsPostBack = False Then
                If Me.m_blnSaveScence = False Then
                    With New Xydc.Platform.DataAccess.dacSystemOperate
                        If .doSaveVisitLogData(strErrMsg, MyBase.UserId, MyBase.UserPassword, Request.UserHostAddress, Request.UserHostName, "sunshineData_houseDetail_compute.aspx", "预售证查询") = False Then
                            GoTo errProc
                        End If
                    End With
                End If
...
errProc:
        ...
        Private Sub doPrint(ByVal strControlId As String)
            ...
            Try
                ...
                If objsystemSunshine.getDataSet_BuildingDetailCompute(strErrMsg, MyBase.UserId, MyBase.UserPassword, Me.htxtComputeQuery.Value, objParameter, Me.m_objDataSet_Compute) = False Then
                    GoTo errProc
                End If
                ...
                Dim strSrcExcelSpec As String = Request.ApplicationPath + Me.m_cstrExcelMBRelativePathToAppRoot + "阳光家缘房产楼盘信息统计表.xls"
