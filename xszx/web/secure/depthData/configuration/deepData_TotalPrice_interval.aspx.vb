...
Namespace Xydc.Platform.web
    ...
    '----------------------------------------------------------------
    ' 命名空间：Xydc.Platform
    ' 类名    ：deepData_TotalPrice_interval
    ' 
    ' 调用性质：
    '   独立运行
    '
    ...
    Partial Public Class deepData_TotalPrice_interval
...
errProc:
        ...
        Private Function getModuleData( _
            ...
            Dim strTable As String = Xydc.Platform.Common.Data.DeepData.TABLE_House_B_TotalPrice_Interval
            ...
            Try
                ...
                With New Xydc.Platform.BusinessFacade.systemDeepdata
                    If .getDeepdataInterVal(strErrMsg, MyBase.UserId, MyBase.UserPassword, strWhere, Me.m_objDeepdataSet, "totalprice") = False Then
                        GoTo errProc
                    End If
                End With
            ...
            Try
                If blnEditMode = False Then
                    ...
                    Else
                        ...
                        intColIndex(0) = objDataGridProcess.getDataGridColumnIndex(Me.grdObjects, Xydc.Platform.Common.Data.DeepData.FIELD_House_B_TotalPrice_Code)
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            If Me.IsPostBack = False Then
                If Me.m_blnSaveScence = False Then
                    Xydc.Platform.SystemFramework.ApplicationLog.WriteAuditPZInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]访问了[总价段]字典！")
                End If
            End If
...
errProc:
        ...
        Private Sub doSave(ByVal strControlId As String)
            ...
            Try
                ...
                objNewData.Add(Xydc.Platform.Common.Data.DeepData.FIELD_House_B_TotalPrice_Code, Me.txtDM.Text)
                ...
                '保存信息
                With New Xydc.Platform.BusinessFacade.systemDeepdata
                    If .doSaveDeepdataInterVal(strErrMsg, MyBase.UserId, MyBase.UserPassword, objOldData, objNewData, Me.m_objenumEditType, "totalprice") = False Then
                        GoTo errProc
                    End If
                End With
                ...
                Select Case Me.m_objenumEditType
                    Case Xydc.Platform.Common.Utilities.PulicParameters.enumEditType.eAddNew
                        Xydc.Platform.SystemFramework.ApplicationLog.WriteAuditPZInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]在[总价段]字典中增加了[" + Me.txtDM.Text + "]！")
                    Case Else
        ...
        Private Sub doDelete(ByVal strControlId As String)
            ...
            Try
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    Dim intPos As Integer
                    intColIndex = objDataGridProcess.getDataGridColumnIndex(Me.grdObjects, Xydc.Platform.Common.Data.DeepData.FIELD_House_B_TotalPrice_Code)
                    For i = intRows - 1 To 0 Step -1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdObjects.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid) = True Then
                            ...
                            '删除处理
                            If objsystemDeepdata.doDeleteDeepdataInterVal(strErrMsg, MyBase.UserId, MyBase.UserPassword, objOldData, "totalprice") = False Then
                                GoTo errProc
                            End If
